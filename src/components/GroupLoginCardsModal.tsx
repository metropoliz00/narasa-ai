...
import { formatGroupUsername } from './GroupManagerModal';
import {
  X,
  Printer,
  Copy,
...
interface GroupLoginCardsModalProps {
  isOpen: boolean;
  onClose: () => void;
  groups: StudentGroup[];
  users: UserProfile[];
...
export const GroupLoginCardsModal: React.FC<GroupLoginCardsModalProps> = ({
  isOpen,
  onClose,
  groups,
  users,
  ...
  const getGroupCredentials = (group: StudentGroup) => {
    const groupUser = users.find(
      (u) => u.id === group.accountUserId || u.email === group.email
    );
    const username = group.username || groupUser?.username || formatGroupUsername(group.name);
    const password = group.password || groupUser?.password || '123456';
  ...
  const handleCopyAllCredentials = () => {
    ...
    targetGroups.forEach((g, idx) => {
      const { username, password } = getGroupCredentials(g);
      text += `[${idx + 1}] ${g.name} (${g.className})\n`;
      text += `Username: @${username}\n`;
      text += `Password: ${password}\n`;
      text += `Ketua: ${g.leaderName || g.memberNames[0] || '-'}\n`;
      text += `Anggota: ${g.memberNames.join(', ')}\n`;
      text += `-------------------------------------------\n`;
    });
    ...
    toast.success(
      'Kredensial Tersalin',
      `Data login ${targetGroups.length} kelompok berhasil disalin ke clipboard.`
    );
    setTimeout(() => setIsCopied(false), 2500);
  };

  // Layout grid classes for print & preview
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5">
      ...
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Header - Screen Only (Hidden on Print) */}
        <div className="p-5 sm:p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 text-white flex items-center justify-between gap-4 shrink-0 no-print">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-lg shadow-blue-600/30 transition-all cursor-pointer active:scale-95"
            >
        ...
        {/* Toolbar & Controls - Screen Only (Hidden on Print) */}
        <div className="p-4 sm:p-5 bg-slate-50 border-b border-slate-200/90 shrink-0 no-print">
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 text-xs">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold text-slate-600 shrink-0">Pilih Kelompok:</span>
              <select
                value={selectedGroupId}
                onChange={(e) => setSelectedGroupId(e.target.value)}
                className="px-3 py-1.5 rounded-xl border border-slate-300 bg-white text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="all">Semua Kelompok ({groups.length} Kartu)</option>
                {groups.map((g) => (
            ...
            <div className="flex items-center gap-4 flex-wrap text-slate-700 font-medium">
              ...
              <button
                ...
                title="Salin teks kredensial kelompok"
              >
                {isCopied ? (
                  <>
          ...
          <div id="narasa-printable-area" className="w-full">
            ...
            {targetGroups.length === 0 ? (
              <div className="text-center py-12 text-slate-400 space-y-2">
                ...
                <p className="text-sm font-semibold">Tidak ada kelompok yang dipilih untuk dicetak.</p>
              </div>
            ) : (
              <div className={getGridClasses()}>
                {targetGroups.map((group, index) => {
                  const { username, password } = getGroupCredentials(group);
                  const accentColor = group.color || '#2563EB';

                  return (
                    <div
                      key={group.id}
                      className="bg-white rounded-2xl border-2 border-dashed border-slate-300 print:border-slate-400 p-4 sm:p-5 relative flex flex-col justify-between shadow-xs print:shadow-none break-inside-avoid print:break-inside-avoid transition-all hover:border-blue-400 group"
                      style={{
                        pageBreakInside: 'avoid'
                      }}
                    >
                      {/* Scissor Cut Line Marks */}
                      <div className="absolute -top-3 right-4 bg-white px-2 text-[10px] font-mono text-slate-400 flex items-center gap-1 select-none pointer-events-none">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded-sm border border-blue-200">
                              KARTU LOGIN
                            </span>
                          </div>
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-[10px] font-semibold text-slate-600 truncate">
                                Ketua: <strong className="text-amber-900">{group.leaderName || group.memberNames[0] || '-'}</strong>
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="p-3 rounded-xl bg-slate-900 text-white print:bg-slate-900 print:text-white space-y-2">
                          <div className="flex items-center justify-between border-b border-slate-700/80 pb-1.5">
                            <span className="text-[10px] font-bold text-blue-300 flex items-center gap-1 uppercase tracking-wider">
                              <KeyRound className="w-3 h-3 text-blue-400" />
                              <span>Akun Login Kelompok</span>
                            </span>
                          ...
                          <div className="bg-slate-50 p-2 rounded-xl border border-slate-200/80 text-[11px]">
                            <div className="flex items-center justify-between text-[10px] font-bold text-slate-600 mb-1">
                              <span className="flex items-center gap-1">
                                ...
                                <span>Anggota Kelompok ({group.memberNames.length} Murid):</span>
                              </span>
                            </div>
                        ...
                        {showInstructions && (
                          <div className="pt-2 border-t border-slate-100 text-[10px] text-slate-500 space-y-0.5">
                            <p className="font-bold text-slate-700 flex items-center gap-1">
                              ...
                              <span>Petunjuk Masuk Bersama Tim:</span>
                            </p>
                            <ol className="list-decimal list-inside pl-0.5 space-y-0.5 text-slate-600">
                              ...
                              <li>Pilih Misi Pembelajaran dan kerjakan secara kolaboratif!</li>
                            </ol>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-3 pt-2 border-t border-slate-100 flex items-center justify-between text-[9px] text-slate-400 font-medium">
                        ...
                        <span>Simpan kartu ini di meja kelompok</span>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="p-4 sm:p-5 bg-white border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0 no-print">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm flex items-center gap-2 shadow-md shadow-blue-500/20 transition-all cursor-pointer active:scale-95"
            >
              <Printer className="w-4 h-4" />
              <span>Cetak Kartu ({targetGroups.length})</span>
            </button>
          </div>
        </div>
