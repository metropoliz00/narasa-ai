      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-15 sm:h-18 gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 shrink-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <span className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-wider font-display bg-gradient-to-r from-[#2563EB] via-[#7C3AED] to-[#EC4899] bg-clip-text text-transparent drop-shadow-2xs select-none">
                  NARASA
                </span>
                <span className="text-[9px] sm:text-[10px] uppercase font-bold tracking-wider px-1.5 sm:px-2 py-0.5 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 text-indigo-700 border border-indigo-200/80 shadow-2xs hidden xs:inline-flex sm:inline-flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
                  AI Kontekstual
                </span>
              </div>
              <p className="text-[10px] sm:text-xs text-slate-500 hidden sm:block font-medium truncate">
          ...
          {isAuthenticated && currentRole === 'student' && studentTab && onSelectStudentTab && (
            <nav className="hidden md:flex items-center gap-1 bg-slate-100/90 p-1 rounded-2xl border border-slate-200/80 shadow-2xs">
              <button
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            {!isAuthenticated && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 border border-slate-200 transition-colors min-h-[40px]"
                title="Lihat Alur Belajar Demo"
              >
                <HelpCircle className="w-4 h-4 text-[#7C5CFC] shrink-0" />
            ...
            {!isAuthenticated ? (
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-bold shadow-xs active:scale-95 transition-all min-h-[40px]"
              >
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className={`flex items-center gap-2 px-2 sm:px-3 py-1.5 rounded-2xl border transition-all text-left cursor-pointer active:scale-98 min-h-[42px] ${
                    isDropdownOpen
                ...
                >
                  ...
                  <div className="text-left leading-tight hidden sm:block max-w-[140px] md:max-w-[180px]">
                    <div className="flex items-center gap-1">
                      <span className="text-xs font-bold text-[#25324B] truncate block">
                ...
                {/* Dropdown Menu Box */}
                {isDropdownOpen && (
                  <div className="fixed sm:absolute right-3 sm:right-0 top-16 sm:top-auto sm:mt-2 w-[calc(100vw-1.5rem)] sm:w-80 max-w-sm bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    <div className="p-2 space-y-1 text-left">
                      ...
                      <button
                        ...
                        className="w-full px-3 py-2.5 rounded-xl text-xs font-medium text-slate-700 hover:text-blue-600 hover:bg-slate-100 flex items-center justify-between transition-colors min-h-[44px]"
                      >
                      ...
                      {(currentUser.role === 'central_admin' || currentUser.role === 'school_admin' || currentUser.role === 'admin') && onNavigateToSchoolSettings && (
                        <button
                          ...
                          className="w-full px-3 py-2.5 rounded-xl text-xs font-medium text-slate-700 hover:text-emerald-700 hover:bg-emerald-50/70 flex items-center justify-between transition-colors cursor-pointer min-h-[44px]"
                        >
                          <div className="flex items-center gap-2.5">
                    ...
                    <div className="p-2 border-t border-slate-100 bg-slate-50/50">
                      <button
                        ...
                        className="w-full px-3 py-2.5 rounded-xl text-xs font-bold text-rose-600 hover:text-rose-700 hover:bg-rose-50 flex items-center justify-between transition-colors min-h-[44px]"
                      >
                        <div className="flex items-center gap-2.5">
